...
1. Create a PDSE (LIBRARY) on your host machine. In our example, we called it **CBSA.JCL.INSTALL** (this
...
>| CBSA.CICSBSA.DSECT        | Will contain the copybooks that are used by the COBOL programs and installation compile jobs (this folder contains Mapset DSECTs too).|
...
The VCAT value for the storage groups must be changed to match your installation's requirements. It seems to be a high level qualifier used for Db2.
...
source code. Expected return code 4.
...
1. Edit job CBSA.JCL.INSTALL(**BANKDATA**) to create the ABNDFILE and
CUSTOMER VSAM files and to execute program BANKDATA which populates the CUSTOMER VSAM file and
ACCOUNT and CONTROL Db2 tables with data (note the PROCTRAN table has no
data in it to start with).

You need to supply @BANK_PREFIX@, @DB2_HLQ@, @BANK_DBRMLIB@ and @BANK_LOADLIB@.

@DB2_HLQ@ is the high level qualifier of the Db2 datasets SDSNLOAD, and the same as "DB2HLQ" as specified in the CBSA.DB2.JCL.INSTALL(**DEFAULT**) file.

@DB2_SUBSYSTEM@ is the name of the Db2 subsystem and the same value as "DB2SYS" in the CBSA.DB2.JCL.INSTALL(**DEFAULT**) file.

@DB2_PLAN@ is the name of the Db2 plan used to bind your application, and the same value as "BANKPLAN" as specified in the CBSA.DB2.JCL.INSTALL(**DEFAULT**) file.

@BANK_PREFIX@ will typically be the same as "CBSAHLQ" from CBSA.CICSBSA.BUILDJCL(DEFAULT)

@BANK_LOADLIB@ will be the same as the value of "CBSAHLQ" from CBSA.CICSBSA.BUILDJCL(DEFAULT), with ".LOADLIB" on the end.

@BANK_DBRMLIB@ will be the same as the value of "CBSAHLQ" from CBSA.CICSBSA.BUILDJCL(DEFAULT), with ".DBRM" on the end.
...
Files are defined as being called DSNAME(CBSA.CICSBSA. as HLQ. If you have changed this then you need to do a global change.
...
2. Edit job CBSA.JCL.INSTALL(**CBSACSD**) to update the DFHCSD with the
...
**CICSTS61** (which is used in the SIT parms for the CICS region).
    ...
    Change @CSD_PREFIX@ to the prefix of the CICS DFHCSD dataset

    Please change @CBSA_INSTALL@ to the name of the dataset.

Then submit it.


3. Check job CBSA.JCL.INSTALL(**CICSTS56**), this is an example CICS region.  You will have your own JCL or PROC
