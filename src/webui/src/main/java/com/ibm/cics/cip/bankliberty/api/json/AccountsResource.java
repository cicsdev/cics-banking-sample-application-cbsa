...
public class AccountsResource extends HBankDataAccess
{
	...
	public Response getAccountsExternal(
			@QueryParam("countOnly") Boolean countOnly)
	{
		...
		HBankDataAccess myHBankDataAccess = new HBankDataAccess();
		myHBankDataAccess.terminate();
		logger.exiting(this.getClass().getName(), GET_ACCOUNTS_EXTERNAL,
				myResponse);
		return myResponse;
	}
		...
		else
		{
			...
			if (myAccounts == null)
			{
				...
				myResponse = Response.status(500).entity(response.toString())
						.build();
				logger.exiting(this.getClass().getName(), GET_ACCOUNTS_INTERNAL,
						myResponse);
				return myResponse;
			}
